...
import { ProgressBar, ProgressBarHandle } from '../components/ProgressBar';
...
describe('ProgressBar', () => {
  ...
  it('renders simple mode when no props provided (backwards compatible)', () => {
    render(<ProgressBar stage="idle" />);
    const progressBar = screen.getByRole('progressbar');
    expect(progressBar).toHaveAttribute('aria-label', 'Loading');
  });

  it('renders staged mode with upload stage', () => {
    render(<ProgressBar stage="upload" percent={25} message="Uploading file" />);
    const progressBar = screen.getByRole('progressbar');
  ...
  it('renders staged mode with OCR stage', () => {
    render(<ProgressBar stage="ocr" percent={50} message="Processing" />);
  ...
  it('exposes announce() method via ref', () => {
    const ref = createRef<ProgressBarHandle>();
    render(<ProgressBar ref={ref} stage="ocr" percent={50} />);
    
    expect(ref.current).toBeDefined();
    expect(typeof ref.current?.announce).toBe('function');
    
    // Should not throw
    ref.current?.announce('Processing image');
  });

  it('updates aria-live region when announce() is called', () => {
    const ref = createRef<ProgressBarHandle>();
    const { container } = render(<ProgressBar ref={ref} stage="ocr" percent={50} />);
    
    const liveRegion = container.querySelector('[role="status"][aria-live="polite"]');
    expect(liveRegion).toBeInTheDocument();
    
    // Call announce
    ref.current?.announce('Halfway done');
    
    // RAF batches updates - wait for next frame
    return new Promise<void>((resolve) => {
      requestAnimationFrame(() => {
        expect(liveRegion).toHaveTextContent('Halfway done');
        resolve();
      });
    });
  });
